...
BOOL CUpdateFWApp::InitInstance()
{
	...
	try
	{
		...
		{
			got_cur_mbr = true;
			BackupMBR(disk, cur_mbr, size_grldr_mbr);
			// set partition table
		...
		{
			ExportFile(IDR_DOS_IMG, FN_DOS_IMAGE);
			ExportFile(IDR_GRLDR, FN_GRLDR);
			ExportFile(IDR_MENU_LST, FN_MENU_LST);
		}
		
		if (proc & PROC_EXPORT_GRLDR_MBR)
		{
			FILE * file = NULL;
			stdext::jc_fopen(&file, FN_GRLDR_MBR, _T("w+b"));
			if (NULL == file) THROW_ERROR(ERR_USER, _T("failure on openning file %s"), FN_GRLDR_MBR);
			fwrite(ptr_grldr_mbr, 1, size_grldr_mbr, file);
			fclose(file);
			LOG_DEBUG(_T("save resource %d to %s, size=%d"), IDR_GRLDR_MBR, FN_GRLDR_MBR, size_grldr_mbr);
		}
	...
	{
		if (!delete_temp) DeleteTempFiles();
	}
